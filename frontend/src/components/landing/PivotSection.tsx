...
function VerticalSlider({ delay, height = "60%", activeColor = "dark", topDot = "black" }: { delay: number, height?: string, activeColor?: "dark" | "orange", topDot?: "black" | "grey" | "orange" }) {
  return (
    <div className="flex flex-col items-center gap-2.5">
      {/* Top dot */}
      <div className={`w-[7px] h-[7px] rounded-full ${topDot === 'black' ? 'bg-[#020202]' :
          topDot === 'orange' ? 'bg-[#FF4D00] shadow-[0_0_8px_rgba(255,77,0,0.6)]' :
            topDot === 'grey' ? 'bg-[#C2C0BD]' : 'bg-transparent'
        }`}></div>

      {/* Track */}
      <div className="relative w-[22px] h-[160px] md:h-[220px] bg-[#EBEBEB] border border-[#DECFBE]/40 shadow-[inset_0_2px_4px_rgba(0,0,0,0.04)] overflow-hidden rounded-full flex flex-col justify-end items-center mx-auto">
        {/* Background Dots */}
        <div className="absolute inset-x-0 top-5 bottom-5 flex flex-col justify-between items-center pointer-events-none z-0">
          <div className="w-[5px] h-[5px] rounded-full bg-[#C2C0BD]"></div>
          <div className="w-[5px] h-[5px] rounded-full bg-[#C2C0BD]"></div>
          <div className="w-[5px] h-[5px] rounded-full bg-[#C2C0BD]"></div>
          <div className="w-[5px] h-[5px] rounded-full bg-[#C2C0BD]"></div>
          <div className="w-[5px] h-[5px] rounded-full bg-[#C2C0BD]"></div>
        ...
        >
          ...
          <div className={`absolute bottom-[4px] w-[14px] top-0 rounded-full ${activeColor === 'orange' ? 'bg-safety-orange' : 'bg-[#3D3A38]'}`}></div>

          {/* Thumb */}
          <div className="relative mt-[1px] w-[14px] h-[14px] rounded-full bg-safety-orange border-[2px] border-white shadow-sm flex items-center justify-center">
            {/* Small inner dot */}
      ...
      {/* Bottom dot */}
      <div className="w-1.5 h-1.5 rounded-full border-[1.5px] border-[#C2C0BD] bg-transparent"></div>
    </div>
  )
}
...
function GrowingBarChart() {
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      {/* Bar Chart */}
      <div className="flex items-end justify-between w-full h-[120px] md:h-[160px] px-1">
        {[...Array(bars)].map((_, i) => {
          const heightPercent = Math.pow(i / (bars - 1), 2.5) * 85 + 15;
          return (
            <motion.div
              key={i}
              className="w-[4px] md:w-[6px] rounded-t-sm bg-safety-orange"
              initial={{ height: '0%' }}
    ...
    <section id="manifesto" className="min-h-[100dvh] w-full bg-dark-base-primary py-8 md:py-16 px-4 md:px-6 lg:px-8 relative z-10 overflow-hidden flex items-center justify-center">
      <div className="w-full max-w-[1400px] mx-auto bg-[#F8F8F8] rounded-xl sm:rounded-[20px] relative overflow-hidden flex flex-col justify-center min-h-[600px] lg:min-h-[700px] shadow-2xl">
        ...
        <div className="w-full h-full pt-8 pb-8 px-6 sm:px-8 md:pt-12 md:pb-12 md:px-10 lg:pt-16 lg:pb-16 lg:px-12 relative z-10 flex flex-col">
          <FadeInStagger className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr] flex-1 min-h-full" staggerDelay={0.15}>

            {/* Column 1 */}
            <FadeInStaggerItem className="flex flex-col justify-between col-span-1 border-b border-[#020202] pb-8 mb-8 md:border-b-0 md:mb-0 md:pb-0 relative z-10 md:pr-10 lg:pr-14">
              <div>
                ...
                <p className="text-pretty font-mono text-[16px] md:text-[18px] leading-[1.6] md:leading-[1.7] text-[#020202] max-w-[34ch]">
              ...
              <h2 className="font-sans font-normal text-[48px] leading-[1.0] tracking-[-0.03em] lg:-ml-1 lg:text-[56px] 2xl:text-[64px] text-balance text-[#020202] mt-16 md:mt-0 mb-[-5px]">
                AI that will work with you, not replace you
              </h2>
            </FadeInStaggerItem>

            {/* Column 2 */}
            <FadeInStaggerItem className="flex flex-col justify-between col-span-1 border-b border-[#020202] pb-10 mb-8 md:border-b-0 md:mb-0 md:border-l md:pl-8 lg:pl-10 md:pb-0 relative z-10">
              <div>
              ...
              <div className="mt-16 flex justify-between items-end w-full max-w-[300px]">
